...
| 2026-06-25 | **Chat quality Phase 5:** LLM-primary chat planner (`sales-planner.ts`) for intent, sub-intent, funnel stage, action, gating, search/tool policy, tenant catalog aliases, contextual price bands, sales recovery CTAs, `salesPlan` eval visibility |
...
*None — all admin screens use real handlers locally.*
...
*None — core auth + team invite flow complete.*
...
- **Chat quality Phases 1–5** — funnel/sub-intent on messages; `cta.ts`; widget cart API; eval suite; LLM-primary chat planner; grounded context reset
